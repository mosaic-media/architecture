...
Arises from [ADR 0132](0132-totp-is-the-second-factor-that-works-everywhere.md),
which needs a TOTP secret encrypted at rest, and answers a question that record
did not ask. It supersedes nothing.

## Context

ADR 0132 acknowledged that "a database breach exposes the factor" and decided no
...
([ADR 0056](0056-redaction-classes-are-the-pii-boundary.md)), and this.
...
- **Losing it is survivable and the survival path already exists.** Every user
  with a second factor also has recovery codes (ADR 0132), which are hashed
...
- **The telemetry salt gains a possible home**, closing something ADR 0056 left
