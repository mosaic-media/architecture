...
Mosaic is an open-source project with a public extension model: a published SDK third parties build modules against ([sdk#1](https://github.com/mosaic-media/sdk/blob/main/docs/adr/0001-sdk-as-public-contract-language.md)), and modules statically compiled into the Platform binary ([platform#4](https://github.com/mosaic-media/platform/blob/main/docs/adr/0004-static-go-module-composition.md)). Making the repositories public required choosing licenses.
...
- **SDUI contract** ([`mosaic-sdui`](https://github.com/mosaic-media/mosaic-sdui), [contracts#3](https://github.com/mosaic-media/contracts/blob/main/docs/adr/0003-sdui-contract-repository.md)) — **Apache-2.0**, for the same reason as the SDK: it is the interface contract a Module builds its UI against, so it must be permissive. It also carries the standard component-definition library and design tokens, permissively so a Module may reuse them freely.
...
- **First-party clients** — **AGPL-3.0-only**, no linking exception. A client (the web [`mosaic-shell`](https://github.com/mosaic-media/mosaic-shell); a future Flutter client for TV, desktop and mobile) is part of the protected first-party product, so it matches the Platform's copyleft — as Jellyfin, Nextcloud and Immich license their own official clients. It carries **no** Module Linking Exception: that permission exists to free a work *compiled into* the Platform binary, and a client links nothing in — it is a separate program talking to the Platform over the network. This extends to first-party client *infrastructure*: the React SDUI runtime [`mosaic-sdui-react`](https://github.com/mosaic-media/mosaic-sdui-react) (`@mosaic-media/sdui-react`, [web#1](https://github.com/mosaic-media/web/blob/main/docs/adr/0001-react-sdui-runtime.md)) and the component storybook [`mosaic-storybook`](https://github.com/mosaic-media/mosaic-storybook) are AGPL-3.0-only too — they are first-party client code, not the contract. The **SDUI contract** a client renders (the technology-agnostic node vocabulary + action envelope that also drives the Flutter client) is, like the SDK, permissive when extracted — so a *third-party* client, in any language, may be any license. First-party polish stays copyleft; the contract stays open.
...
**Copyleft SDK.** *Rejected:* it would drag every compiled-in module into copyleft through the dependency, defeating [sdk#1](https://github.com/mosaic-media/sdk/blob/main/docs/adr/0001-sdk-as-public-contract-language.md)'s open-ecosystem purpose.
