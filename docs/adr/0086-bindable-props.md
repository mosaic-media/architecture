...
- **An object is a binding only with exactly that one key and a non-empty string
  ...
  ([ADR 0084](0084-vocabulary-negotiation-and-deliberate-degradation.md)) and for
...
That is the ergonomics redesign [ADR 0044](0044-contracts-protobuf-workspace.md)
...
- **Bindings resolve against a scope the client owns.** The server cannot know
  ...
  ADR 0084 has no equivalent for props; that gap is real and unaddressed.
