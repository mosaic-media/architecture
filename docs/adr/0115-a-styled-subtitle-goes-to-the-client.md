...
[ADR 0114](0114-a-subtitle-track-has-a-form.md) gave a styled subtitle track two
...
3. **The viewer's choice becomes three-valued** — `plain`, `client`, `burn` —
   replacing ADR 0114's boolean, whose `true` meant `burn`. **`client` is the
...
4. **The client echoes the server's choice and does not rank.** It draws the
   ...
   ([ADR 0112](0112-language-is-a-persons-preference.md)); re-deciding it in a
...
ADR 0114 gave: WebVTT has `line`, `position` and `align`, and none of `\pos` at
