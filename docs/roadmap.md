...
### The active thread: a live, fully server-driven client — **done end to end (slices 1–2)**
...
**Where this leaves the client thread:** the app is a live, fully server-driven client end to end. Natural follow-ups, none blocking: **socket reconnection/resume** (slice 2 does a single connect — a drop shows Standby, no auto-retry yet); **back/history navigation** (the server-emitted top bar has no back control, and history isn't modelled over the socket); **finer region updates** (an `input` re-renders the whole content region, not just the results grid — the protocol supports targeting a narrower region); and **server-side input debounce** (the client debounces ~220 ms; the Platform coalescing is the backstop ADR 0032 calls for and is not yet added).
