...
on the action lane [ADR 0106](0106-the-pre-session-action-lane.md) adds. A third
...
[ADR 0098](0098-claiming-an-unclaimed-server.md)'s two doorway states unchanged.
**Date:** 2026-07-26
...
- **Four clients** ([ADR 0041](0041-cross-client-transport-two-lane-rpc.md)) —
  ...
  clothes.
- **Negotiation exists** ([ADR 0084](0084-vocabulary-negotiation-and-deliberate-degradation.md)):
...
*without* a session ([ADR 0061](0061-one-client-transport.md)).

Five things follow.

**The request carries the same vocabulary declaration `Attach` carries.**
Version plus supported primitive and action sets, so
[ADR 0084](0084-vocabulary-negotiation-and-deliberate-degradation.md) applies to
...
[ADR 0098](0098-claiming-an-unclaimed-server.md): the setup tree while the
...
for a Platform that is *down* ([ADR 0060](0060-the-supervisor-observes-independently.md));
