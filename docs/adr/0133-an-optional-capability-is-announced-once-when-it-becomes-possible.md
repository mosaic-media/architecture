# 133. An optional capability is announced once, when it becomes possible

**Status:** Proposed. Nothing is built. The rule is stated generally because Mosaic already has several capabilities only some installs can have, but the two it governs today are passkeys ([ADR 0131](0131-passkeys-are-an-optional-layer-on-a-public-origin.md)) and TOTP ([ADR 0132](0132-totp-is-the-second-factor-that-works-everywhere.md)).
**Date:** 2026-08-10

## Context

[ADR 0131](0131-passkeys-are-an-optional-layer-on-a-public-origin.md) made
passkeys available only to installs with a public origin, and
[ADR 0132](0132-totp-is-the-second-factor-that-works-everywhere.md) made TOTP
...
- **Announcing something an install cannot have.** A `.local` Mosaic that
  ...
  [ADR 0058](0058-telemetry-storage-retention-and-expert-mode.md) already met
...
- **Detecting is not setting.** ADR 0131 refuses to infer the relying-party id
  ...
  them, and collapsing them reintroduces exactly the silent failure ADR 0131
  rejected.
...
- **This says nothing about capabilities a user lacks *permission* for**, which
  is a different question with an existing answer (ADR 0058's rule: an
