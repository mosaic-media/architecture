...
Extends [ADR 0068](0068-one-principal-many-credentials.md), which named four
...
[ADR 0131](0131-passkeys-are-an-optional-layer-on-a-public-origin.md) settled
...
- **It is a factor, not a credential, and that is not pedantry — it changes
  where it goes in the code.** ADR 0068's diagram has four credentials each
...
- **It is offered during onboarding, as its own optional step.** It works from
  ...
  The wizard behind [ADR 0098](0098-claiming-an-unclaimed-server.md) is four
...
- **A new store and a new domain type**: the shared secret is credential material
  and must be classified accordingly (ADR 0056) — never logged, never rendered
