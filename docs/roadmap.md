...
### The active thread: modules as typed providers, and two content planes — **backend done (slices 1–3); SDUI remaining (slice 4)**
...
4. **SDUI screens — remaining, [ADR 0029](adr/0029-sdui-emit-side.md) written.** The **user search screen** and the **admin collection browser** are the *first real screens the Platform emits* ([ADR 0023](adr/0023-server-driven-ui-and-the-shell.md)). The emit-side is decided in [ADR 0029](adr/0029-sdui-emit-side.md): a GraphQL `screen(name, params)` query returns a `UINode` tree built by a Platform-owned screen registry with the [`mosaic-sdui`](https://github.com/mosaic-media/mosaic-sdui) Go producer binding, and a screen's actions name the slice-3 GraphQL operations (search, browse-catalogs, materialise — all now existing). Then point the Shell's two routes at `screen(...)` instead of mock payloads. Modules contribute data, not screens ([ADR 0008](adr/0008-sdk-as-public-contract-language.md)).
