...
[ADR 0037](0037-completing-the-stremio-source-surface.md) defined the `subtitles`
...
[ADR 0113](0113-subtitles-are-a-rendition.md)'s embedded path cannot help there —
...
1. **Asked at play, not stored at import.** A subtitle URL is perishable in the
   ...
   [ADR 0049](0049-resolution-cache-and-capability-classes.md) already names for
   streams.
...
4. **The origin fetches; the URL never reaches a client.** A module resolves and
   the Platform serves ([ADR 0045](0045-playback-consumer-and-media-origin.md)),
...
6. **None is ever default.** The release's own tracks are what a viewer's
   preference was resolved against ([ADR 0112](0112-language-is-a-persons-preference.md));
...
- **This is the one subtitle path a direct-played release can have.** An embedded
  track needs a playlist to hang an HLS rendition off, so ADR 0113's path is
...
- **The module is named in each label.** Two sources answering for one title is
  ...
  from.
- **`v1.Subtitle` still cannot say a track is forced**, so ADR 0112's forced
