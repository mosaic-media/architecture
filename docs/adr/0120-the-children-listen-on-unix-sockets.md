...
[ADR 0005](0005-supervisor-guarantees-an-interface.md) says the Supervisor is
...
[ADR 0077](0077-go-plugin-as-the-extension-harness.md)). Third-party code gets
...
([ADR 0101](0101-the-pre-session-bootstrap.md)), and the limiter keys on the
...
- **`X-Forwarded-For` becomes trustworthy, and the Platform reads it.** This is
  ...
  ADR 0101's per-peer ceiling starts working again as a result — and it would
...
box (ADR 0063), not only an image. A deployment that adds namespaces on top of
this is welcome to.
