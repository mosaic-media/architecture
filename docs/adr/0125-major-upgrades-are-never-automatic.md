...
[ADR 0124](0124-artefacts-are-paired-by-contract-major.md) for something to
check against.
...
- **Crossing a contract major is always major**, whatever the artefacts' own
  version numbers say ([ADR 0124](0124-artefacts-are-paired-by-contract-major.md)).
...
- **A staged Generation is a finding**
  ([ADR 0119](0119-operational-findings-are-durable-state.md)): the register is
...
- **"Check" becomes a thing that runs on its own**, so the rate limit and the
  freshness gap in ADR 0124 stop being hypothetical.
