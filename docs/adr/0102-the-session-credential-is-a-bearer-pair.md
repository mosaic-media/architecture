...
- **The access token is minutes long and opaque.** Not a JWT — for
  [ADR 0068](0068-one-principal-many-credentials.md)'s reason and not for
...
[ADR 0068](0068-one-principal-many-credentials.md)'s grounds. It also buys
...
- A refresh token used by an attacker and then by the legitimate user (or the
  ...
  fault.
- Passkeys ([ADR 0068](0068-one-principal-many-credentials.md)) change what
