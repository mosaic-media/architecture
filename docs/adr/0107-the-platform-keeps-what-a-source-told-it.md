...
on every render" and extends [ADR 0028](0028-virtual-and-materialized-content.md)'s
single materialising capability the same way
[ADR 0073](0073-stream-resolution-is-decoupled-from-metadata-provenance.md) and
[ADR 0075](0075-the-artwork-provider-role.md) already did.
**Date:** 2026-07-27
...
**A library item's detail screen was blank.** [ADR 0034](0034-rich-metadata-preview.md)
...
credential shared by every default install ([ADR 0105](0105-project-credentials-in-official-builds.md))
...
maintenance pass ([ADR 0104](0104-the-library-is-built-from-rules.md)) re-merged
artwork and topped up Parts and never extended the tree. That is the *first*
example ADR 0104 gives of what a self-maintaining library is for, and it did not
...
document per node with the time it was fetched. [ADR 0071](0071-content-artwork-is-stored-on-the-node.md)
...
module-owned and the Platform never interprets it ([ADR 0013](0013-object-graph.md));
...
the Platform building a tree, which [ADR 0028](0028-virtual-and-materialized-content.md)
gave to the materialising capability — and the ground for taking part of it back
is exactly [ADR 0073](0073-stream-resolution-is-decoupled-from-metadata-provenance.md)'s:
...
- **A metadata provider is now asked about content on a schedule** rather than
  when somebody looks. That is the load shift [ADR 0104](0104-the-library-is-built-from-rules.md)
