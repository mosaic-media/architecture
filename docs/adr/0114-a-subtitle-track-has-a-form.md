...
[ADR 0113](0113-subtitles-are-a-rendition.md) delivers subtitles as HLS
...
3. **A typeset track is flattened by default and burned on request.** The
   ...
   [ADR 0112](0112-language-is-a-persons-preference.md) — **off by default**, and
...
5. **`off` never burns**, and this is the strongest form of ADR 0112's rule.
...
- **Filtergraph values are escaped twice, and getting it wrong fails silently.**
  ...
  ([ADR 0111](0111-the-playlist-is-a-nominal-grid.md)) and `-t` (ADR 0113).
