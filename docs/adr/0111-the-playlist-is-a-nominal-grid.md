...
[ADR 0110](0110-the-segment-length-is-measured.md) reasoned from one correct
...
The window makes no difference. ADR 0110 cites "93 ms" as evidence the probe is
...
This is the real error. ADR 0110 assumed a playlist must describe what one
...
[ADR 0108](0108-the-origin-is-a-pipe-only-where-it-must-be.md)'s `-ss` before
...
1. **The playlist is a nominal grid** computed from the probed duration and a
   ...
   whole running time is scrubbable from the first frame — ADR 0109's point that
...
**Measure the interval and match it** ([ADR 0110](0110-the-segment-length-is-measured.md)).
...
- **Three records now govern one slice, and two are superseded.** That is the
  cost of reasoning where measuring was available: ADR 0110 was written from an
