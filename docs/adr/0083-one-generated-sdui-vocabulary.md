...
[ADR 0024](0024-primitives-and-definitions.md) split the interface into two
tiers: **primitives**, native code each client implements, and **definitions**,
compositions expressed as data. [ADR 0082](0082-components-are-authored-only-in-the-contract.md)
...
- **Six of the ten wire action kinds were unauthorable from Go**, and one
  ...
  [ADR 0061](0061-one-client-transport.md) while surviving in the proto, fields
  and all.
...
**`query` returns to the contract as a different action.** ADR 0061 removed a
