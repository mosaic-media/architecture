...
### The next thread: a live, fully server-driven client — **ADRs written, build not started**

Wiring the Shell exposed two ways the current client-server model still cheats on the SDUI thesis, and both were decided in ADRs ahead of the build:

- **[ADR 0031](adr/0031-server-owned-app-shell.md) — the Shell is a pure renderer; the app shell is server-emitted.** The Shell's sidebar/top bar/chrome are hardcoded React today; only the content area is a payload. This makes them a payload too: the Platform emits the *whole* app shell (nav rail, top bar, a named content region) and the Shell owns nothing but the connection lifecycle, a tiny set of client-only meta states (*connecting / unreachable / standby*), and the renderer. Without a server it shows standby, never chrome. Needs a few SDUI vocabulary additions (an app-shell/layout with named regions, a nav rail) and subsumes the mock chrome screens. Refines [ADR 0029](adr/0029-sdui-emit-side.md): a screen renders *into* the shell's content region, not as a whole page.
- **[ADR 0032](adr/0032-live-session-websocket.md) — the live session over a bidirectional WebSocket.** Request/response is superseded by one persistent, authenticated socket: the client streams *intents* (navigate, keystroke input, invoke), the server pushes *UI updates* into named regions (replace/append/patch), plus toasts and shell mutations. This is the region-update protocol [ADR 0029](adr/0029-sdui-emit-side.md) deferred. It buys **search-as-you-type** (input streams up, results stream into the region — debounced server-side so the upstream addons aren't hammered) and **server push** (alerts, live library changes) — the two things request/response can't do; raw latency was explicitly *not* the justification on a localhost server. Costs named honestly: the server becomes stateful (live sessions), owns reconnection/ordering/backpressure, and the protocol must be versioned.

**Build sequence (the chosen order):** ADRs (done) → the server-owned app shell over the current request/response (smaller, settles the chrome model) → swap the transport to the live WebSocket. The app shell is the initial push over that socket, so [ADR 0031](adr/0031-server-owned-app-shell.md) lands first and [ADR 0032](adr/0032-live-session-websocket.md) changes the channel under it.
