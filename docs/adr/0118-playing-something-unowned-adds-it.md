...
effort: [ADR 0028](0028-virtual-and-materialized-content.md) admits exactly two
crossings between virtual and materialised, while
[ADR 0046](0046-playback-state-is-platform-owned.md) keys progress by node — and
...
1. **Materialise at play *start*, not at commitment.** This dissolves the
   ...
   there is a node to key it against, so ADR 0046 needs no change and ADR 0028's
...
3. **It authorises as an import.** A viewer without `content.import` is refused
   ...
   the authority that curates the library ([ADR 0069](0069-privilege-cannot-escalate.md)),
...
([ADR 0049](0049-resolution-cache-and-capability-classes.md)), subtitle
...
- **The library gains things people bounced off.** Accepted, and named. The
  library-rule maintenance pass ([ADR 0104](0104-the-library-is-built-from-rules.md))
...
- **An added work with no releases is not an error.** The import succeeded and
  ...
  ([ADR 0116](0116-a-preference-is-a-default-an-override-is-a-sitting.md)).
- **A viewer who cannot import still cannot play unowned content**, and now sees
  ...
  ADR 0069 made to the Add button when the first ordinary account pressed it and
  got nothing.
