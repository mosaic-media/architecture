...
**Status:** Partly built, and partly superseded: the import boundary was widened again by [ADR 0123](0123-the-supervisor-answers-the-platforms-client-surface.md); the rest stands. The contract dependency and the emitter it was for are built; the two images, the DIY path and onboarding's module step are not.
**Date:** 2026-08-09

Amends [ADR 0080](0080-deployment-topologies.md)'s topology table (the supervised
container splits in two, and the unsupervised arrangement is named as a path
rather than left implied). Settles a question
[ADR 0005](0005-supervisor-guarantees-an-interface.md) opened and did not answer:
...
[ADR 0063](0063-platform-binary-built-by-ci.md) for the artefacts and
[ADR 0081](0081-extension-installation-is-user-initiated-and-persistent.md) for
...
**What is in the image.** [ADR 0004](0004-supervisor-as-host-manager.md) opens
with "add Mosaic to a Docker Compose file and start", and
[ADR 0080](0080-deployment-topologies.md) names a Container topology — but
neither says whether the database is inside it. PostgreSQL is deliberately
outside the Generation boundary
([ADR 0011](0011-platform-transports-events.md)'s reload classes put the DSN in
...
**Whether the Supervisor is required.** ADR 0063 makes installation "downloading
...
**How the Supervisor emits SDUI.** ADR 0005 decides that the Supervisor emits
...
import, which makes ADR 0005 unimplementable as written. That is not a defect in
...
cannot decode ([ADR 0050](0050-probing-and-the-per-stream-playback-decision.md)); absent, it relays unprobed
...
identical bytes (ADR 0080), both reach the same Platform, and a module installed
either way is installed by the Platform on a user's request (ADR 0081).
...
The Supervisor never fetches a module (ADR 0079). The step remains a surface over
[ADR 0081](0081-extension-installation-is-user-initiated-and-persistent.md)'s
...
experience "install a database first", which is the thing ADR 0063 removed a Go
...
an experienced homelab owner actually want; ADR 0080 already rejected
