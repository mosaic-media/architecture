# 129. The upgrade channel is the handoff and the register

**Status:** Built. The Supervisor checks the catalogue on a schedule and spools an offer, polls `GET /upgrade` and carries out what it finds; the Platform raises `upgrade_available`, offers `apply_upgrade`, records the request and settles it by comparing `MOSAIC_GENERATION_ID` against the version asked for. Not built: ADR 0125's automation *policy* on top of it — every upgrade is a person pressing something today, which is that record's Manual level and its safest.
**Date:** 2026-08-09

Closes the question [ADR 0125](0125-major-upgrades-are-never-automatic.md)
...
ADR 0125 decided the *policy* — three levels, and a contract change is never
...
- **The findings spool** is how the Supervisor tells the Platform things
  ([ADR 0119](0119-operational-findings-are-durable-state.md)). It already
  carries what the Supervisor learned while the Platform was not there, and
  ADR 0125 already anticipated its use here: "A staged Generation is a finding:
...
- **The handoff reports the pending request, and the Supervisor polls it.**
  ...
  ADR 0120 keeps off the public port precisely so Generation state is not
...
- **Failure travels back on the spool, as a finding.** An upgrade that would not
  ...
  did not install.
- **The policy of ADR 0125 sits on top of this and is not built here.** This
  ...
  artefact's ([ADR 0127](0127-the-monitored-version-is-the-contract-not-the-artefact.md)).
...
[ADR 0123](0123-the-supervisor-answers-the-platforms-client-surface.md) has the
