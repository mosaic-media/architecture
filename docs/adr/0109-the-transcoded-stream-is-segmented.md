...
[ADR 0108](0108-the-origin-is-a-pipe-only-where-it-must-be.md) measured that a
...
measurement ADR 0108 made. Reading every video packet's timestamp over a remote
...
2. **A segment request is a position, and the restart machinery is reused.**
   ...
   `-copyts`. Those are ADR 0108's flags and its arithmetic, re-keyed from a byte
...
5. **The web client adopts a media framework. This is
   [ADR 0070](0070-the-web-player-is-the-browser.md)'s condition firing, not
   ADR 0070 being reversed.** That record chose a bare `<video>` element and said
...
above: over a remote source it costs the whole file, which is the cost ADR 0108's
...
- **The byte-addressing code is deleted, not left beside its replacement**:
  ...
  with their tests. `ShouldRemux` — dead code ADR 0108 named and deliberately did
...
- **A slower-than-realtime encode is still slower than realtime.** ADR 0108's
