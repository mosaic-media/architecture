...
[ADR 0128](0128-opentelemetry-is-the-telemetry-implementation.md) and reinstates
two of the three grounds
[ADR 0059](0059-modules-observe-through-the-sdk.md) originally rejected it on.
ADR 0128's central decision — OpenTelemetry is Mosaic's telemetry
implementation, in every process — **stands unchanged**.

## Context

ADR 0128 gave the SDK its first dependency: four OpenTelemetry API modules,
...
That is the distinction ADR 0128 missed, and ADR 0059 had already drawn it:
...
ADR 0128 overrode that on the grounds that OpenTelemetry is vendor-neutral and
...
Of ADR 0059's three reasons, the second (a module could reach the configuration
...
- **OpenTelemetry remains the implementation, everywhere.** Nothing about what
  Mosaic emits changes, no module changes beyond a version bump, and ADR 0128's
...
- **The same rule settles the crypto question without a second argument.** The
  Platform's secret facility ([ADR 0134](0134-the-install-key.md) and its
...
keep nothing shared. *Rejected.* It is the arrangement ADR 0128 was written to
...
- **The SDK's `go.mod` returns to a module line and a Go version**, and
  ...
  did before ADR 0128 widened it to an allowlist.
...
- **ADR 0059's first and third rejections stand again**, and the record should
