...
Completes [ADR 0005](0005-supervisor-guarantees-an-interface.md)'s "one emitter,
many renderers" by removing the second *source* a client would otherwise have
had. Partly supersedes
[ADR 0121](0121-two-supervised-images-and-a-diy-path.md): the Supervisor's import
boundary widens from one module to two.

## Context

ADR 0005 has the Supervisor emit its own SDUI so that a person is told what is
...
- **Credential calls are refused, not faked**, and refused as `Unavailable`
  ...
  refresh chain (ADR 0102), and signing somebody out because their server
