...
[ADR 0109](0109-the-transcoded-stream-is-segmented.md) decided that a
...
real timing. ADR 0109 specified a computed playlist *and* fMP4, which is the one
...
2. **Where the video is copied, the source chooses and the origin measures.** A
   ...
   This is **not** [ADR 0109](0109-the-transcoded-stream-is-segmented.md)'s
...
4. **The playlist stays complete and stays `VOD`.** This is the half of
   ADR 0109 that was right and it is the requirement everything else serves: every
...
- **The head probe joins the existing probe** (ADR 0050) rather than replacing
