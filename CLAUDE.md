...
- **Adding a top-level page means editing `nav:` in `mkdocs.yml` and `TOP_LEVEL` in `scripts/build_pdfs.py`.** That friction is deliberate. It is a reminder that a fifth page needs justifying.
- **Adding a decision record means editing `nav:` and nothing else.** Its PDF is derived from `nav:`, and the export refuses to run if a record is missing from `nav:` or `nav:` names a record that does not exist. Hand-maintaining a second list is what left the export covering fourteen of 135 records, and `unreachable-capability.md` covering none, with nothing to report it.
- **A record's heading carries no number** — `# Capabilities do not own stores`, not `# 12. …`. The number lives in the filename and in `nav:`. It was in the heading too, which is a third copy that nothing checks: a renumbering that misses it builds green. Leaving it out also makes the page's anchor stable when the number changes.
