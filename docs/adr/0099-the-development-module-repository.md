...
and its trusted key), [ADR 0079](0079-the-platform-manages-extension-modules.md)
(the Platform is the actor for the extension lifecycle) and
[ADR 0081](0081-extension-installation-is-user-initiated-and-persistent.md) (an
extension is installed by a user at runtime and re-adopted across restarts).
Touches [ADR 0080](0080-deployment-topologies.md)'s SSRF containment, because
reaching a local repository means reaching a private address. It changes nothing
about what a shipped Platform does.

## Context

ADR 0081 took the extension modules out of the Platform binary, and it was
...
env table, a hosting panel, a compose file copied from a forum post. ADR 0080
...
is the SSRF shape the guard exists for (ADR 0080). So the override also selects
...
(ADR 0065).** *Rejected for this purpose*, though it remains the right shape for
...
- **A rebuilt index does not reach an already-installed module**, and that is
  ADR 0081 working rather than a gap: boot re-adopts the *pinned* bytes from
