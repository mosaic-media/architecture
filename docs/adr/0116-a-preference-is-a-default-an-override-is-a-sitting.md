...
**Item 2: selection is invisible.** [ADR 0048](0048-stream-selection-against-a-client-profile.md)
...
[ADR 0112](0112-language-is-a-persons-preference.md) made language a per-user
...
2. **It ranks and does not resolve.** Resolving a candidate is a call to an
   ...
   ([ADR 0049](0049-resolution-cache-and-capability-classes.md)); resolving
...
(ADR 0049), which is where that failure belongs.
...
([ADR 0047](0047-player-as-client-primitive.md)).
...
- **The picker screen carries releases and not yet tracks.** The audio and
  ...
  (ADR 0113), so the visible gap is audio. That is an
