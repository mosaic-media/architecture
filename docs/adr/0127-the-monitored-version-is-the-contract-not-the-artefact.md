...
Supersedes [ADR 0126](0126-before-1-0-the-caution-window-shifts-by-one.md)
wholly. Partly supersedes
[ADR 0124](0124-artefacts-are-paired-by-contract-major.md) and
[ADR 0125](0125-major-upgrades-are-never-automatic.md): their shape stands and
...
ADR 0125 made caution depend on the size of a change in **the artefact's own
version** — a major of the Platform or the Shell is never activated unattended.
ADR 0126 then asked how to read that version before 1.0 and shifted the window
...
ADR 0126 went further and asserted that a contract minor bump before 1.0 is not
...
- **A contract change is the risky upgrade**, at any artefact version — it is
  ...
  unattended, exactly as ADR 0125 describes, with the subject corrected.
...
- **The same component decides compatibility.** ADR 0124 pairs the Platform and
...
and it is the shape ADR 0125 left behind. A Platform can certainly change what
...
it is ADR 0126's position and the `ClientProfile` incident is the counterexample.
...
- **Automation becomes useful before 1.0, which ADR 0126 could not manage.**
...
- **Cutting patch releases stops being necessary.** ADR 0126's consequence — that
