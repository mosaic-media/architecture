...
Completes the condition [ADR 0059](0059-modules-observe-through-the-sdk.md) set
...
ADR 0059 gave modules levelled logging with classified fields and spans, and
...
That reasoning was right and its premise has changed. [ADR 0128](0128-opentelemetry-is-the-telemetry-implementation.md)
...
([ADR 0055](0055-instrument-at-the-seams.md), seam 8), the context carries the
...
matter because ADR 0128 had to swallow a pre-1.0 logs API to land:
...
- **Metric attributes are classified exactly as log fields are** (ADR 0056),
...
- **Cardinality is bounded per scope, for the life of the process, and an
  ...
  version of ADR 0059's record quota and it needed a different lifetime — a
...
- **A refused instrument is reported through the channel the module is already
  ...
  silently-discarding counter ADR 0059 refused to ship, arrived at by a typo.
- **The harness carries both calls.** Three modules run out of process
  ([ADR 0077](0077-go-plugin-as-the-extension-harness.md)), so a bridge that
...
and it is what ADR 0059 refused* — correctly. It is worth naming because the
...
and they will often already have it. *Rejected*, for the reasons ADR 0128 gave
...
the ownership ADR 0059 placed with the Platform. It also loses classification at
...
open-vs-closed test (ADR 0015) asks whether Platform code branches on it; this
...
- **The SDK's OpenTelemetry allowlist goes from three modules to four.** The
  ...
  the *stable* half of OpenTelemetry, unlike the logs API ADR 0128 had to take
  at `v0.21.0`.
- **`v1.Telemetry` gained two methods, which breaks every implementer.** Pre-1.0
  minor, sanctioned by [ADR 0127](0127-the-monitored-version-is-the-contract-not-the-artefact.md);
...
- **Nothing exports to a collector yet.** The Platform holds a reader rather
  ...
  ADR 0128's benefit — everything Mosaic emits being readable by tooling nobody
...
- **A module still cannot be *made* to instrument.** `log.Printf` remains a
  ...
  repository (ADR 0059's own stated weakness). Metrics inherit that: the surface
